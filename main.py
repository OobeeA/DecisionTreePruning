...
# recursive decision tree learning algorithm using information gain
# inputs:
...
# Function plots the tree when a tree is trained on a certain dataset
def plot_tree(dataset):
    tree = decision_tree_learning(dataset, 0)
    # Plotting the sample tree top-down
    plot_tree_top_down(tree)
    plt.show()

# function plots the tree when the tree is cleaned on a clean dataset
#plot_tree(clean_dataset) 
...
def display_dataset_confusion_matrix(dataset):
    # make the clean tree
    tree = decision_tree_learning(dataset, 0)
    # get the confusion matrix of the full clean dataset
    confusion_matrix = calculate_confusion_matrix(dataset, tree)
    display_confusion_matrix(confusion_matrix)

# Displays the confusion matrix of a tree made 
# display_dataset_confusion_matrix(clean_dataset)
...
# Computes the cross validation classification metrics for any given dataset
def display_metrics_for_dataset(dataset):
    confusion_matrix, average_accuracy = tenfold(dataset)
    display_confusion_matrix(confusion_matrix)
    accuracy, precisions, recalls, f1_measures = compute_metrics(confusion_matrix)
    display_metrics_table("Dataset Tenfold", accuracy, precisions, recalls, f1_measures)


#display_metrics_for_dataset(clean_dataset) # prints the metrics for the clean dataset
#display_metrics_for_dataset(noisy_dataset) # prints the metrics for the noisy dataset
...
def prune(dataset):
    ...
    for i in range(10):
        ...
        for val_index in range(9):
            ...
            accuracy_before = evaluate_accuracy(trained_tree, test_dataset) # compute the accuracy before any pruning 
...
def display_results_for_prune_data(dataset):
    confusion_matrix, avg_depth_before, avg_depth_after, avg_acc_before, avg_acc_after = prune(dataset)

    accuracy = display_info_table("Stats", confusion_matrix, avg_depth_before, avg_depth_after, avg_acc_before, avg_acc_after)
    display_confusion_matrix(confusion_matrix)
    accuracy, precisions, recalls, f1_measures = compute_metrics(confusion_matrix)
    display_metrics_table("Metrics", accuracy, precisions, recalls, f1_measures)


#display_results_for_prune_data(clean_dataset)
#display_results_for_prune_data(noisy_dataset)

a, b = tenfold(clean_dataset)
print(a)
print(b)

display_confusion_matrix(a)